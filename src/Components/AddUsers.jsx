...
export default function Addusers() {
    ...
    const googleHandler = () => {
        signInWithPopup(auth, provider)
            .then(result => {
                ...
                console.log(user)
                const currentUser = { name: user.displayName, email: user.email }
    ...
    return (
        <div className=" w-1/3 mx-auto my-32 p-20 border-2 bg-violet-100 shadow-2xl shadow-gray-500 rounded-lg">
            ...
            <div className="flex items-center pt-4 space-x-1 mt-3">
                <div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
                <p className="px-3 text-sm dark:text-gray-400">Submit with social platform</p>
                <div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
            </div>
            <div className="flex justify-center space-x-4">
                <button onClick={googleHandler} aria-label="Log in with Google" className="p-3 rounded-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-6 h-6 fill-current text-gray-600">
